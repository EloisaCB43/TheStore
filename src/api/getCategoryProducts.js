import axios from "axios";
const URL_API = "https://restaurant-menu-eloisa.herokuapp.com/api/products";

const getCategoryProducts = async (idCategory, finallyCallback) => {
  const URL_API_CATEGORIES = `${URL_API}?populate=img&filters[$and][0][category][name][$eq]=${idCategory}`;
  try {
    const response = await axios.get(URL_API_CATEGORIES, {
      headers: {
        Authorization:
          "Bearer ",
      },
    });
    return response.data.data;
  } catch (err) {
    console.log(err);
  } finally {
    finallyCallback();
  }
};

export default getCategoryProducts;
